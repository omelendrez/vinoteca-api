...
module.exports = {
  ...
  add: (req, res) => {
    Stock.save(req.body) // Le decimos al modelo que ejecute la función "save" y le pasamos lo que el cliente posteó en la ruta
      .then(stock => res.json(stock)) // Aquí todo salió bien, devolvemos  al cliente lo que nos devolvió el modelo
  ...
  update: (req, res) => {
    Stock.update(req.body, req.params.id) // El controlador le está enviando los datos nuevos y el id de la empresa a modificar
  ...
  getAll: (req, res) => {
    Stock.getAll() // El controlador le dice al modelo que ejecute la función getAll
  ...
  getById: (req, res) => {
    Stock.getById(req.params.id) // Le decimos al modelo que ejecuta la función getById y le pasamos el id que estaba en la url
      .then(stock => res.json(stock))
      .catch(err => res.status(500).json(err))
  }
}