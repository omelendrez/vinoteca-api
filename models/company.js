...
module.exports = {
  ...
  save: async company => { // El controlador ha ejecutado esta función pasado el objeto que le envió el cliente a la respectiva ruta
    const [fields, values] = await insertFieldsFromModel(company) // Función helper que genera el código SQL para ejecutar un INSERT
    return new Promise(async (resolve, reject) => { // Creamos una Promise, que nos permite actuar de manera diferente si hubo errores o no
      const sql = `INSERT INTO company (${fields}) VALUES (${values})` // La lista de fields y values es generada automáticamente
  ...
  update: async (company, id) => { // El controlador nos pasa los datos que envió el cliente (datos de la empresa y su id)
    const [fields] = await updateFieldsFromModel(company) // Otra función helper que genera SQL para ejectuar un UPDATE
    return new Promise(async (resolve, reject) => { // Creamos una nueva Promise
      const sql = `UPDATE company SET ${fields} WHERE id=?` // Preparamos el SQL
      pool.executeQuery(sql, [id], (err, results, fields) => { // Enviamos el SQL y el id (estamos modificando un solo registro) a mysql
        if (err) return reject(err) // Si hubo errores devolvemos el error y terminamos acá
        const sql = `SELECT * FROM company WHERE id=?` // Si no hubo errores creamos un query para traer los datos de este registro
  ...
  getAll: () => {
    return new Promise((resolve, reject) => { // Creamos una nueva Promise
      const sql = 'SELECT * FROM company;' // Preparamos el query
  ...
  getById: id => { // Por eso nos pasa el id
    return new Promise((resolve, reject) => { // Creamos una nueva Promise
      const sql = 'SELECT * FROM company WHERE id=?;' // Preparamos el query
