...
module.exports = {
  ...
  update: async (orderDetails, id) => { // El controlador nos pasa los datos que envió el cliente (datos de la empresa y su id)
    ...
    return new Promise(async (resolve, reject) => { // Creamos una nueva Promise
      ...
      pool.executeQuery(sql, [id], (err, results, fields) => { // Enviamos el SQL y el id (estamos modificando un solo registro) a mysql
        if (err) return reject(err) // Si hubo errores devolvemos el error y terminamos acá
        const sql = `SELECT * FROM order_details WHERE id=?` // Si no hubo errores creamos un query para traer los datos de este registro
  ...
  getAll: () => {
    return new Promise((resolve, reject) => { // Creamos una nueva Promise
      const sql = 'SELECT * FROM order_details;' // Preparamos el query
  ...
  getById: id => { // Por eso nos pasa el id
    return new Promise((resolve, reject) => { // Creamos una nueva Promise
      const sql = 'SELECT * FROM order_details WHERE id=?;' // Preparamos el query
